...
mmpCapGetDeviceInfo()
{
    ...
    Capctxt->ininfo.TopFieldPol = MMP_TRUE;
    ...
    void)
{
    ...
    isSignalChange = CaptureModuleDriver_IsSignalStable(CaptureModuleDrivers);   // mmpADV7180IsSignalStable();
...
ithCapInitialize()
{
    ...
    for (i = ITH_GPIO_PIN34; i <= ITH_GPIO_PIN45; i++)   //GPIO34 ~ GPIO45  for capture setting
    {
...
//int ithCap_DestroyHandle(ITE_CAP_HANDLE *ptHandle)
//int ithCaptureRun()
//int ithCap_CreateHandle(ITE_CAP_HANDLE *ptHandle, uint8_t *startaddr)
