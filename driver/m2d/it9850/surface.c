...
#define REG_FILL_SET_ITM(regs, mtx) \
    {   \
        ...
        m00                  = (int32_t)(mtx.m[0][0] * (1 << 15));   \
        m01                  = (int32_t)(mtx.m[0][1] * (1 << 15));   \
        m02                  = (int32_t)(mtx.m[0][2] * (1 << 15));   \
        m10                  = (int32_t)(mtx.m[1][0] * (1 << 15));   \
        m11                  = (int32_t)(mtx.m[1][1] * (1 << 15));   \
        m12                  = (int32_t)(mtx.m[1][2] * (1 << 15));   \
        m20                  = (int32_t)(mtx.m[2][0] * (1 << 15));   \
        m21                  = (int32_t)(mtx.m[2][1] * (1 << 15));   \
        m22                  = (int32_t)(mtx.m[2][2] * (1 << 15));   \
        regs->regITMR00.data = (m00 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR01.data = (m01 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR02.data = (m02 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR10.data = (m10 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR11.data = (m11 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR12.data = (m12 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR20.data = (m20 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR21.data = (m21 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
        regs->regITMR22.data = (m22 & GFX_REG_ITMR_MASK) << GFX_REG_ITMR_OFFSET;    \
    }
    ...
    int32_t        lineWidth)
{
    ...
    for (t = 0.0; t <= 1.0; t += 0.01)
    {
        ...
        result = _SurfaceDrawPixelAA(surface, round(x), round(y), lineColor, lineWidth);
    }
    ...
    GFXPageFlowType type)
{
    ...
    int32_t       minX, minY;
    int32_t       newDestWidth = dest->width - dx - 1;
    ...
    if (dest)
    {
        //printf("dest(%d %d) (%d %d)\n", dx, dy, newDestWidth, dest->height);
        REG_FILL_DST_SURFACE(regs, dest, dx, dy + minY, newDestWidth, dest->height - dy - minY - 1);
    }
    ...
    // Mask
    REG_FILL_MASK_SURFACE(regs, dest, dx, dy + minY, newDestWidth, dest->height - dy - minY - 1);
    ...
    int32_t    lineWidth)
{
    ...
    result     = _SurfaceDrawPixel(surface, round(x), round(y), lineColor, lineWidth);
