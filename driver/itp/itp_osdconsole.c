...
static int OsdConsolePutchar(int c)
{
    ...
    if (c == '\n')
        ithFlushDCacheRange(osd_base, lcd_line_length * CFG_LCD_HEIGHT);
...
void OsdConsoleClear(void)
{
	...
	memset ((char *)osd_base,
		0xAA,
		lcd_line_length * CFG_LCD_HEIGHT);
...
{
    lcd_line_length = (((CFG_LCD_WIDTH * CFG_LCD_BPP + 31) & ~31) >> 3);
    osd_addr = itpVmemAlloc(lcd_line_length * CFG_LCD_HEIGHT);
    osd_base = (void*) ithMapVram(osd_addr, lcd_line_length * CFG_LCD_HEIGHT, ITH_VRAM_WRITE);

    OsdConsoleClear();
    while (ithLcdCursorIsUpdateDone());
    ithLcdCursorSetWidth(CFG_LCD_WIDTH);
    ithLcdCursorSetHeight(CFG_LCD_HEIGHT);
...
static int OsdConsoleWrite(int file, char *ptr, int len, void* info)
{
    ...
    {
        uint16_t value = ithReadRegH(ITH_LCD_DHWC_UPDATE_REG);
        if ((value & (1<<ITH_LCD_DHWC_UPDATE_BIT)) == 0)
            break;
    }
