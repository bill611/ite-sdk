...
#define CONSOLE_ROWS		(CFG_LCD_HEIGHT / ITP_FONT_HEIGHT)
#define CONSOLE_COLS		(CFG_LCD_WIDTH / ITP_FONT_WIDTH)
...
static int LcdConsolePutchar(int c)
{
    ...
    if (c == '\n')
    {
        ithFlushDCacheRange(lcd_base_a, lcd_line_length * CFG_LCD_HEIGHT);
        ithFlushDCacheRange(lcd_base_b, lcd_line_length * CFG_LCD_HEIGHT);
    #ifdef DRAW_VIDEO_BUF        
        ithFlushDCacheRange(lcd_base_c, lcd_line_length * CFG_LCD_HEIGHT);
    #endif
    }
    return c;
}
...
void LcdConsoleInit(void)
{
    ...
    lcd_base_a = (unsigned char*) ithMapVram(ithLcdGetBaseAddrA(), lcd_line_length * CFG_LCD_HEIGHT, ITH_VRAM_WRITE);
    lcd_base_b = (unsigned char*) ithMapVram(ithLcdGetBaseAddrB(), lcd_line_length * CFG_LCD_HEIGHT, ITH_VRAM_WRITE);
#ifdef DRAW_VIDEO_BUF    
    lcd_base_c = (unsigned char*) ithMapVram(ithLcdGetBaseAddrC(), lcd_line_length * CFG_LCD_HEIGHT, ITH_VRAM_WRITE);
#endif
    lcd_color_fg	= 0xFFFF;
    lcd_color_bg	= 0;
	console_col		= 0;
...
void LcdConsoleClear(void)
{
	...
	ithMemset16(lcd_base_a,
		lcd_color_bg,
		lcd_line_length * CFG_LCD_HEIGHT / sizeof (uint16_t));
