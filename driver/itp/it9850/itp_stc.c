...
{    
    ...
    ithTimerDisable(ITH_TIMER1);
    /* Start with tick timer interrupts disabled... */
    ithIntrDisableIrq(ITH_INTR_TIMER1);

    /* Clear any pending tick timer interrupts... */
    ithTimerClearIntr(ITH_TIMER1);
    ithIntrClearIrq(ITH_INTR_TIMER1);
    ithIntrRegisterHandlerIrq(ITH_INTR_TIMER1, StcIntrHandler, NULL);
    ithIntrSetTriggerModeIrq(ITH_INTR_TIMER1, ITH_INTR_EDGE);
    ithTimerCtrlEnable(ITH_TIMER1, ITH_TIMER_UPCOUNT);
    ithTimerCtrlEnable(ITH_TIMER1, ITH_TIMER_PERIODIC);

    /* Enable the tick timer interrupt... */
    ithIntrEnableIrq(ITH_INTR_TIMER1);

    /* Calculate timer compare value to achieve the desired tick rate... */
    ithTimerSetCounter(ITH_TIMER1, 0);
    ithTimerSetMatch(ITH_TIMER1, 0xfffff);
