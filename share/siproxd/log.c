...
void log_set_pattern(unsigned int pattern) {
    printf("[sip proxy] log_set_pattern %d \n",pattern);
    pattern = (DBCLASS_RTP | DBCLASS_SIP | DBCLASS_PROXY_RESPONSE);
...
void log_tcp_listen(void) {
   ...
   /* set non-blocking */
   flags = fcntl(debug_listen_fd, F_GETFL,0);
   if (flags < 0) {
...
/*   fprintf(stderr, "%2.2i:%2.2i:%2.2i %s%s:%i ", tim->tm_hour,
   ...
   fprintf(stderr, "%2.2i:%2.2i:%2.2i :%i ", tim->tm_hour,
           tim->tm_min, tim->tm_sec,  line);
...
char* log_win32(const char *fmt, va_list ap)
{
...
//    msg = log_win32(format,ap);
   // printf("sip proxy %s ",msg);
