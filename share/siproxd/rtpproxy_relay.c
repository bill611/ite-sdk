...
#define SIP_PROXY_STACK_SIZE      (255 * 1024)
...
//   pthread_attr_getstacksize (&attr, &stacksize);
   ...
   if (configuration.thread_stack_size > 0) {
      stacksize = SIP_PROXY_STACK_SIZE; //configuration.thread_stack_size*1024;
...
//      pthread_testcancel();
      ...
      for (i=0;(i<RTPPROXY_SIZE) && (num_fd>0);i++) {
         ...
         if ( (rtp_proxytable[i].rtp_con_rx_sock != 0) &&
            ...
            if (count > 0) {
               ...
               if (rtp_proxytable[i].rtp_con_tx_sock != 0) {
                  ...
                  strcpy(remip1, utils_inet_ntoa(dst_addr.sin_addr) );
                  // Don't dejitter RTCP packets 
              //    printf("sendto [%d] %d ,to %s:%d, %d, read(fd) %d , local port %d \n",i,rtp_proxytable[i].dest_sock,remip1,rtp_proxytable[i].dest_port,count,rtp_proxytable[i].rtp_con_rx_sock,rtp_proxytable[i].local_port);
...
#else
          memcpy(&dst_addr.sin_addr.s_addr,
              &rtp_proxytable[i].dest_ipaddr,
                 sizeof(struct in_addr));
          dst_addr.sin_port= htons(rtp_proxytable[i].dest_port);

           strcpy(remip1, utils_inet_ntoa(dst_addr.sin_addr) );
                  /* Don't dejitter RTCP packets */
           printf("sendto [%d] %d ,%s:%d, %d, read(fd) %d , local port %d \n",i,rtp_proxytable[i].dest_sock,remip1,rtp_proxytable[i].dest_port,count,rtp_proxytable[i].rtp_con_rx_sock,rtp_proxytable[i].local_port);

          sts = sendto(rtp_proxytable[i].dest_sock, rtp_buff,
                       count, 0, (const struct sockaddr *)&dst_addr,
                       (socklen_t)sizeof(dst_addr));
#endif
        ...
        // strcpy(remip1, utils_inet_ntoa(rtp_proxytable[i].remote_ipaddr));
        //     strcpy(lclip1, utils_inet_ntoa(rtp_proxytable[i].local_ipaddr));
        //printf("i %d rem ip %s local %s \n",i,remip1,lclip1);
                  ...
                  strcpy(remip2, utils_inet_ntoa(dst_addr.sin_addr));
        //    printf("sendto [%d] %d ,%s:%d, %d, read(fd) %d , local port %d \n",i,rtp_proxytable[i].rtp_tx_sock,remip2,rtp_proxytable[i].dest_port,count,rtp_proxytable[i].rtp_rx_sock,rtp_proxytable[i].local_port);
                  sts = sendto(rtp_proxytable[i].rtp_tx_sock, rtp_buff,
                               count, 0, (const struct sockaddr *)&dst_addr,
                               (socklen_t)sizeof(dst_addr));
    #else
            memcpy(&dst_addr.sin_addr.s_addr,&rtp_proxytable[i].dest_ipaddr,sizeof(struct in_addr));
            dst_addr.sin_port= htons(rtp_proxytable[i].dest_port);

            strcpy(remip2, utils_inet_ntoa(dst_addr.sin_addr));
            printf("sendto %d %d,%s:%d, %d,read(fd) %d, local port %d\n",i,rtp_proxytable[i].dest_sock,remip2,rtp_proxytable[i].dest_port,count,rtp_proxytable[i].rtp_con_rx_sock,rtp_proxytable[i].local_port);
            sts = sendto(rtp_proxytable[i].dest_sock, rtp_buff,
                       count, 0, (const struct sockaddr *)&dst_addr,
                       (socklen_t)sizeof(dst_addr));
    #endif
...
int rtp_relay_start_fwd (osip_call_id_t *callid, client_id_t client_id,
   ...
   for (i=0; i<RTPPROXY_SIZE; i++) {
      ...
      if (rtp_proxytable[i].rtp_rx_sock &&
          ...
          */
       // it changes real remote ip and port,so mark these code
#if 0 
         /* Port number */
         ...
         if (memcmp(&rtp_proxytable[i].remote_ipaddr, &remote_ipaddr,
            ...
            memcpy (&rtp_proxytable[i].remote_ipaddr, &remote_ipaddr,
                     sizeof(remote_ipaddr));
            printf("RTP IP address changed %d \n",i);
         }
#endif
#ifdef USE_DEJITTER
   ...
   time(&rtp_proxytable[freeidx].timestamp);
    ...
    printf("add rtp relay To %s:%d,index %d  rx %d\n",utils_inet_ntoa(rtp_proxytable[freeidx].remote_ipaddr), rtp_proxytable[freeidx].remote_port,freeidx,sock);
   DEBUGC(DBCLASS_RTP,"rtp_relay_start_fwd: addr=%s, port=%i, sock=%i, "
          "freeidx=%i, input data dejitter buffer=%i usec", 
          utils_inet_ntoa(local_ipaddr), port, sock, freeidx, dejitter);
...
/*   printf("[Sip proxy] rtp local addr %s port %d,remote addr %s port %d \n",utils_inet_ntoa(rtp_proxytable[freeidx].local_ipaddr),rtp_proxytable[freeidx].local_port
        ,utils_inet_ntoa(rtp_proxytable[freeidx].remote_ipaddr),rtp_proxytable[freeidx].remote_port);*/
...
#if 0
     memcpy(&rtp_proxytable[rtp_proxytable_idx].dest_ipaddr,&rtp_proxytable[j].remote_ipaddr,sizeof(struct in_addr));
         memcpy(&rtp_proxytable[j].dest_ipaddr,&rtp_proxytable[rtp_proxytable_idx].remote_ipaddr,sizeof(struct in_addr));
         rtp_proxytable[rtp_proxytable_idx].dest_port = rtp_proxytable[j].remote_port;
         rtp_proxytable[j].dest_port = rtp_proxytable[rtp_proxytable_idx].remote_port;
         rtp_proxytable[j].dest_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
            if (!strncmp(remip2, "10.", 3)) {
                 
                int sts=0;
                dst_addr.sin_family = AF_INET;
                memcpy(&dst_addr.sin_addr.s_addr,
                &rtp_proxytable[j].dest_ipaddr,
                sizeof(struct in_addr));
                dst_addr.sin_port= htons(rtp_proxytable[j].dest_port);

                /* Don't dejitter RTCP packets */
                printf("sendto [%d] %d ,%s:%d, %d, read(fd) %d , local port %d \n",j,rtp_proxytable[j].dest_sock,remip2,rtp_proxytable[j].dest_port,sts,rtp_proxytable[j].rtp_con_rx_sock,rtp_proxytable[j].local_port);

                sts = sendto(rtp_proxytable[j].dest_sock, " ", 1, 0, (const struct sockaddr *)&dst_addr, (socklen_t)sizeof(dst_addr));
                printf("%d \n",sts);
         ...
         printf("connected entry %i (fd=%i, %s:%i->%s:%i) <-> entry %i (fd=%i, %s:%i->%s:%i) \n",
                             j, rtp_proxytable[j].rtp_rx_sock,
                             lclip1, rtp_proxytable[j].local_port,
                             remip1, rtp_proxytable[j].remote_port,
                             rtp_proxytable_idx,
      ...
      /*   if (strcmp(remip2, "10.93.67.180") == 0){
            
            strcpy(remip2, "91.121.209.194");
            utils_inet_aton(remip2,&rtp_proxytable[rtp_proxytable_idx].remote_ipaddr);
            printf("%s \n",utils_inet_ntoa(rtp_proxytable[rtp_proxytable_idx].remote_ipaddr));
         }*/
         printf("index %d rx %d tx %d  remote %s \n",j,rtp_proxytable[j].rtp_rx_sock,rtp_proxytable[j].rtp_tx_sock,remip1);
