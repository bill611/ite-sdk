...
extern void linphone_core_run_stun(int port1,int port2);
...
int siproxd_main (int argc, char *argv[])
{
   ...
   char configfile[64]="A:/siproxd";       /* basename of configfile */
...
/*
   if (nStun==0){
    linphone_core_run_stun(7074,7076);
    nStun++;
   }
*/
...
//      printf("[SIPROXD]received data \n");
      ...
      {
        char* pch;
        char temp[128] = "sip:nickhan@sip.linphone.org";
          if (strstr(buff,"sip:toto@192.168.111.47")){
            pch =strstr(buff,"sip:toto@192.168.111.47");
            memcpy(pch,temp,28);
          }
      }
#endif
    ...
    if (MSG_IS_STATUS_2XX(ticket.sipmsg)) {
        printf("OK \n");
