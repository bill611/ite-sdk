...
#define PROXY_INTERNAL_INDOOR "192.168.191."
...
int  proxy_check_invitation_contact(sip_ticket_t *ticket)
{
   ...
   if (strstr(ticket->raw_buffer,PROXY_CONTACT))
   {
       ...
       //
       pch =strstr(cTemp,PROXY_INTERNAL_INDOOR);
       if (pch){
           pch = strrchr(cTemp,'.');
           for (i=0;i<3;i++){
               if ( isdigit(pch[1+i]) ){
   ...
   for (j=0;j<register_user_number;j++)
   {
    ...
    &&&*/
		  if ( !strcmp(ticket->sipmsg->sip_method,"BYE") &&
			 !strcmp(request->from->url->host, siproxd_in_indoor)){
...
int proxy_rewrite_invitation_body(sip_ticket_t *ticket, int direction,int nRTP){
   ...
   if (!strncmp(sdp->c_connection->c_addr, "192.168.191.220", 15))
   {
      memcpy(sdp->c_connection->c_addr,"123.194.235.203",15);

   }
   if (!strncmp(sdp->o_addr, "192.168.191.220", 15))
   {
      memcpy(sdp->o_addr,"123.194.235.203",15);
...
{ /* just dump the buffer */
   ...
   if (sdp->c_connection && sdp->c_connection->c_addr) {
      ...
      if (strcmp(sdp->c_connection->c_addr, "0.0.0.0") != 0) {
        ...
        if (direction == DIR_INCOMING){
            sprintf(sdp->c_connection->c_addr, "%s", "123.194.235.203");
         } else {
   ...
   if (sdp->o_addrtype && sdp->o_addr) {
      if (strcmp(sdp->o_addrtype, "IP4") != 0) {
        ...
        if (direction == DIR_INCOMING){
            sprintf(sdp->o_addr, "%s", "123.194.235.203");
         } else {
