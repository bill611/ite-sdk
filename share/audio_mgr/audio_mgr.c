...
#endif      
            memcpy(gOutBuffer + gOutReadPointer, audioMgr.sampleBuf, audioMgr.ptNetwork.nLocalFileSize);
            I2S_DA32_SET_WP(gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize);
        } else if (gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize<= 128*1024) {
            memcpy(gOutBuffer + gOutReadPointer, audioMgr.sampleBuf, audioMgr.ptNetwork.nLocalFileSize);
            I2S_DA32_SET_WP(gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize);        
        } else if (gOutReadPointer+audioMgr.ptNetwork.nLocalFileSize > 128*1024) {
            memcpy(gOutBuffer, audioMgr.sampleBuf, audioMgr.ptNetwork.nLocalFileSize);
            I2S_DA32_SET_WP(audioMgr.ptNetwork.nLocalFileSize);
        } else {
