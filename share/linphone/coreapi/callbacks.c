...
static void call_ringing(SalOp *h){
    ...
    if (md==NULL){
		...
		call_have_received = false; //doorbell hangout first
    }else{
        /*accept early media */
...
static void call_accepted(SalOp *op){
    ...
    if (call==NULL || call_have_received==true){
        ms_warning("No call to accept.");
        return ;
    }
    else
	    call_have_received = true;
...
static void call_terminated(SalOp *op, const char *from){
	...
	call_have_received = false; //client hangout first
