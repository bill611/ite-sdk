...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (sdpLine != NULL) {
  	if(strncmp(sdpLine, "m=video", 7) && strncmp(sdpLine, "m=audio", 7))
  	{
  		//printf("skip sdpLine = %s\n", sdpLine);
...
MediaSubsession::MediaSubsession(MediaSession& parent)
  : serverPortNum(0), sink(NULL), miscPtr(NULL),
    ...
    fSessionId(NULL) {
  rtpInfo.seqNum = 0; rtpInfo.timestamp = 0; rtpInfo.infoIsNew = True;
}
...
#if defined(CFG_WFD_SOURCE_DEVICE) || defined(CFG_WFD_SINK_DEVICE)
...
#else /* #if defined(CFG_WFD_SOURCE_DEVICE) || defined(CFG_WFD_SINK_DEVICE) */
