...
bool ituExecActions(struct ITUWidgetTag* widget, ITUAction* actions, ITUEvent ev, int arg)
{
    ...
    for (i = 0; i < ITU_ACTIONS_SIZE; i++)
    {
        ...
        else
        {
            ...
            else if (action->ev == ITU_EVENT_ENTER || action->ev == ITU_EVENT_LEAVE || action->ev >= ITU_EVENT_CUSTOM)
            {
                if (arg && (action->action == ITU_ACTION_FUNCTION || action->action == ITU_ACTION_INPUT))
