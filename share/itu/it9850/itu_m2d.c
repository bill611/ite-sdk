    ...
    uint8_t constantAlphaValue)
{
    ...
    if (surf->flags & ITU_CLIPPING)
    {
        gfxSurfaceSetClip(destSurf->m2dSurf, x, y, x+w, y+h);
    }
    ...
    float scaleY)
{
    ...
    if (dest->clipping.x+dest->clipping.width > CFG_LCD_WIDTH && dest->flags & ITU_CLIPPING)
    {
    ...
    uint8_t constantAlphaValue)
{
    ...
    if (dest->clipping.x + dest->clipping.width >= CFG_LCD_WIDTH || ((dest->flags & ITU_CLIPPING) && angle == 0))
    {
        _SetClipRectRegion(dest, dx, dy, dw, dh);
    }
