...
static void ulaw_enc_process(MSFilter *obj){
    ...
    while (ms_bufferizer_read(bz,buffer,size_of_pcm)==size_of_pcm){
		...
		int size = adpcm_coder(audio_buf, buffer, size_of_pcm);
        mblk_t *o=allocb(size,0);
        memcpy(o->b_wptr, audio_buf, size);
        o->b_wptr+=size;
		...
		// }
#else
        mblk_t *o=allocb(size_of_pcm,0);
        memcpy(o->b_wptr, buffer, size_of_pcm);
        o->b_wptr+=size_of_pcm;
#endif
...
static void ulaw_dec_process(MSFilter *obj){
    ...
    while((m=ms_queue_get(obj->inputs[0]))!=NULL){
        ...
        msgpullup(m,-1);
#ifdef ADPCM
		short audio_buf[2048] = {0};
		int audio_size = adpcm_decoder(m->b_rptr,audio_buf,(m->b_wptr-m->b_rptr)*2);
        o=allocb(audio_size,0);
        mblk_meta_copy(m, o);
        memcpy(o->b_wptr, audio_buf, audio_size);
        o->b_wptr+=audio_size;
        ...
        mblk_meta_copy(m, o);
        memcpy(o->b_wptr, m->b_rptr, (m->b_wptr-m->b_rptr)*2);
        o->b_wptr+=(m->b_wptr-m->b_rptr)*2;
