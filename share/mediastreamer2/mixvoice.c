...
static void mix_set_level(MSFilter *f, void *arg){
    Mixdata *d=(Mixdata*)f->data;
#if (CFG_CHIP_FAMILY == 9910)
    d->mixlevel = *((double*)arg);
#else
    d->mixlevel = *((float*)arg);
#endif
    printf("d->mixlevel = %f \n",d->mixlevel);
    return 0;
}

static int mix_set_loop(MSFilter *f, void *arg){
    Mixdata *d=(Mixdata*)f->data;
    d->loop = *((int*)arg);
    return 0;
}
 ...
 static MSFilterMethod mixvoice_methods[]={
    ...
    {   MS_MIXVOISE_SET_LOOP    ,   mix_set_loop},
    {   0               ,   NULL        }
}; 
