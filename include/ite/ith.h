...
static inline int32_t ithMemDbgGetFlag(ITHMemDbgModule module)
{
    ...
    {
        if (status & (1 << ITH_MEMDBG_FLAG_STATUS0_BIT))
        {
    ...
    {
        if (status & (1 << ITH_MEMDBG_FLAG_STATUS1_BIT))
        {
 ...
 * @param ctrl the controls to enable.
 */
static inline void ithUartFifoCtrlEnable(ITHUartPort port, ITHUartFifoCtrl ctrl)
{
    ithSetRegBitA(port + ITH_UART_FCR_REG, ctrl);
}
 ...
 * @param ctrl the controls to disable.
 */
static inline void ithUartFifoCtrlDisable(ITHUartPort port, ITHUartFifoCtrl ctrl)
{
    ithClearRegBitA(port + ITH_UART_FCR_REG, ctrl);
}
 ...
 */
static inline void ithUartSetTxTriggerLevel(
    ITHUartPort port,
    ITHUartTriggerLevel level)
{
	
    ithSetRegBitA(port + ITH_UART_FCR_REG, ITH_UART_FCR_TXFIFO_RESET_BIT);
	ithWriteRegMaskA(port + ITH_UART_FCR_REG, level << ITH_UART_FCR_TXFIFO_TRGL_BIT, ITH_UART_FCR_TXFIFO_TRGL_MASK);
}
 ...
 */
static inline void ithUartSetRxTriggerLevel(
    ITHUartPort port,
    ITHUartTriggerLevel level)
{
	
    ithSetRegBitA(port + ITH_UART_FCR_REG, ITH_UART_FCR_RXFIFO_RESET_BIT);
	ithWriteRegMaskA(port + ITH_UART_FCR_REG, level << ITH_UART_FCR_RXFIFO_TRGL_BIT, ITH_UART_FCR_RXFIFO_TRGL_MASK);
}
